...
interface YenConverterProps {
  ...
  /** Callback: passa o valor em Yen (não convertido) para o formulário do pedido */
  onUseYenValue?: (yenValue: string) => void;
  /** Callback: notifica a cotação JPY→BRL atual sempre que ela muda */
...
export function YenConverter({ onUseValue, onUseYenValue, onRateChange }: YenConverterProps) {
  ...
  // Notifica o formulário do pedido sempre que a cotação mudar
  useEffect(() => {
    onRateChange?.(rateState.rate);
  ...
  return (
    <div className="converter-panel">
      ...
      {/* Botão usar valor em Yen (base para a conversão WISE) */}
      {onUseYenValue && (
        <button
          className="converter-use-btn"
          disabled={jpyNum <= 0}
          onClick={() => onUseYenValue(jpyNum.toFixed(2))}
        >
          ✦ Usar {jpyNum > 0 ? fmtJPY(jpyNum) : "¥ —"} como valor do item
        </button>
      )}
    </div>
  );
